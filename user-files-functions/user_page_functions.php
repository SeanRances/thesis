<?php
function performance_today($emp_id)
{
  ...
  $year = date('Y');
  $perf = "";
  $today_perf_query = "SELECT * FROM performance_record WHERE record_day = '$day' AND record_month = '$month' AND record_year = '$year' AND emp_id = '$emp_id'";
  $today_perf_result = mysqli_query($con,$today_perf_query);

  if($today_perf_result)
  {
    if(mysqli_num_rows($today_perf_result))
    {
      while($row = mysqli_fetch_array($today_perf_result))
      {
        $perf = $row['emp_QA'] . "||" . $row['emp_CPH'];
      }
      return $perf;
    }
  }
...
function get_name_view_emp($emp_id)
{
  include '../includes/connection.php';
